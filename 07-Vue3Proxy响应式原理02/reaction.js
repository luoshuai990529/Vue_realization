import { registerReactionForOperation, releaseReaction, getReactionsForOperation } from './store.js';

/** 依赖收集栈 */
const reactionStack = [];

/** 依赖收集 在get操作的时候要调用 */
export function registerRunningReaction(operation) {
...
/** 把函数包裹为观察函数 */
export function runReactionWrap(reaction, fn, context, args) {
  // 已经取消观察了 就直接执行原函数
...
/** 传入的观察函数是否正在运行 */
export function isRunning(reaction) {
